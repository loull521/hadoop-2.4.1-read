...
public class ContainerLocalizer {
  ...
  public int runLocalization(final InetSocketAddress nmAddr)
    ...
    // load credentials
    initDirs(conf, user, appId, lfs, localDirs);//初始化目录结构
    ...
    final LocalizationProtocol nodeManager =//LocalizationProtocol协议客户端代理
    ...
    try {
      exec = createDownloadThreadPool();//下载线程池
        ...
        case LIVE:
          ...
          for (ResourceLocalizationSpec newRsrc : newRsrcs) {
            if (!pendingResources.containsKey(newRsrc.getResource())) {
              pendingResources.put(newRsrc.getResource(), cs.submit(download(//submit提交下载任务
