...
public class ContainerManagerImpl extends CompositeService implements
  ...
  private void startContainerInternal(NMTokenIdentifier nmTokenIdentifier,
    ...
    ContainerLaunchContext launchContext = request.getContainerLaunchContext();//container启动信息
    ...
    Container container =
        new ContainerImpl(getConfig(), this.dispatcher, launchContext,
          credentials, metrics, containerTokenIdentifier);//创建ContainerImpl状态机
    ...
    try {
      if (!serviceStopped) {
        ...
        Application application =
            new ApplicationImpl(dispatcher, user, applicationID, credentials, context);//创建ApplicationImpl状态机，有时候没必要创建的
        if (null == context.getApplications().putIfAbsent(applicationID,
          application)) {//如果是这个applicationImpl不在NMContext维护的app集合中，也就是每个NM都维护了一个application
          LOG.info("Creating a new application reference for app " + applicationID);

          dispatcher.getEventHandler().handle(
            new ApplicationInitEvent(applicationID, container.getLaunchContext()
              .getApplicationACLs()));//发送事件
        }

        dispatcher.getEventHandler().handle(
          new ApplicationContainerInitEvent(container));//发送事件
