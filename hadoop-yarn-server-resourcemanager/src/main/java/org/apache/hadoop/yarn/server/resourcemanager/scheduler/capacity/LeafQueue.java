...
public class LeafQueue implements CSQueue {
  ...
  assignContainers(Resource clusterResource, FiCaSchedulerNode node) {
    ...
    for (FiCaSchedulerApp application : activeApplications) {
      //遍历activeApplications，fifo选择app。如果设置了activeApplications集合的比较器为DRF，就不是FIFO策略了。
      ...
      synchronized (application) {
        ...
        // Schedule in priority order 按优先级的顺序
        for (Priority priority : application.getPriorities()) {
          ...
          if (!needContainers(application, priority, required)) {//这个方法没看懂
            continue;
          }
          ...
          if (Resources.greaterThan(
            ...
            // Done
            return assignment;//如果分配了，就返回，不再遍历activeApplications
          } else {
  ...
  private CSAssignment assignContainersOnNode(Resource clusterResource, 
    ...
    if (nodeLocalResourceRequest != null) {//如果这个app在这个priority和nodeName上的请求不为空
      assigned = 
  ...
  private Resource assignContainer(Resource clusterResource, FiCaSchedulerNode node, 
    ...
    if (availableContainers > 0) {
      ...
      // Inform the application，分配了container，要让schedulerAppAttempt知道
      ...
      // Inform the node，要让schedulerNode知道
