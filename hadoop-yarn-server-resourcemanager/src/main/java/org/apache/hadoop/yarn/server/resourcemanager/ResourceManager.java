...
/**
 * RM主要包括以下几个模块：
 * 1、用户交互模块：ClientRMService，AdminService，WebApp
 * 2、NM管理模块
 * 3、AM管理模块
 * 4、Application管理模块
 * 5、状态机管理模块
 ...
 */
  ...
  /**
   * 两类services：
   * AlwaysOn services 由RM实例自己管理
   * Active Services 由内部类RMActiveService实例管理
   */

  /**
   * "Always On" services. Services that need to run always irrespective of
   * the HA state of the RM.
   * 一直运行的service，不论RM是否active
   */
  @VisibleForTesting
  protected RMContextImpl rmContext;//RM context
  private Dispatcher rmDispatcher;//中央异步分配器
  @VisibleForTesting
  protected AdminService adminService;//针对管理员的对外服务，管理集群，比如动态更新节点列表，更新ACL列表，更新队列信息等。
   ...
   * in Active state.
   */
  protected RMActiveServices activeServices;//在active rm上运行的service
  protected RMSecretManagerService rmSecretManagerService;

  //默认使用org.apache.hadoop.yarn.server.resourcemanager.scheduler.capacity.CapacityScheduler调度器
  protected ResourceScheduler scheduler;//资源调度模块
  private ClientRMService clientRM;				//针对普通用户的对外服务，处理来自客户端的RPC请求，比如提交应用程序，获取应用程序状态等。
  protected ApplicationMasterService masterService;//AMS，处理来自AM的请求，主要包括注册和心跳。
  protected NMLivelinessMonitor nmLivelinessMonitor;	//监控NM是否还活着，如果NM在一定时间内（10min）内没有心跳，则认为已死
  protected NodesListManager nodesListManager;	//维护正常节点和异常节点，黑名单和白名单
  protected RMAppManager rmAppManager;//管理应用程序的启动和关闭,还有状态管理
  protected ApplicationACLsManager applicationACLsManager;//管理应用程序访问权限，包括查看和修改权限。修改主要用于修改应用程序优先级，杀死应用程序等
  //QueueACLsManager:Check if the user has permission to perform the operation. 
  //If the user has QueueACL.ADMINISTER_QUEUE permission, this user can view/modify the applications in this queue
  protected QueueACLsManager queueACLsManager;
  private WebApp webApp;	//web界面，是一个轻量级嵌入式web框架
  private AppReportFetcher fetcher = null;
  protected ResourceTrackerService resourceTracker;//处理ResourceTracker协议。处理来自NM的请求，包括注册和心跳两种。组册时发送NM的全部资源状况信息
  ...
  protected void serviceInit(Configuration conf) throws Exception {
    ...
    rmDispatcher = setupDispatcher();//生成一个一步dispatcher，并注册RMFatalEvent
    addIfService(rmDispatcher);//不是所有的dispatcher都是service，只有AsyncDispatcher是service
    ...
    createAndInitActiveServices();//创建和初始化RMActiveServices
    //前面把alwaysOn service添加到service列表中，这个函数添加avtive services到RMActiveService管理的service列表中,并初始化
  ...
  /**
   * RMActiveServices handles all the Active services in the RM.<br>
   * 管理所有的active services，不包括一直开启的service
   */
  @Private
  class RMActiveServices extends CompositeService {
    ...
    //AM收到RM新分配的Container后，必须在一定时间(10min)内启动，否则回收。由这个对象来处理这个过程
    ...
    protected void serviceInit(Configuration configuration) throws Exception {
      ...
      try {
        scheduler.reinitialize(conf, rmContext);//加载Scheduler的配置文件
  ...
  protected void serviceStart() throws Exception {
    ...
    if (this.rmContext.isHAEnabled()) {
      ...
      transitionToActive();//启动activeServices中的service
    }
