...
public class CapacityScheduler extends AbstractYarnScheduler
  ...
  private CSQueue root;//根节点队列

  static final Comparator<CSQueue> queueComparator = new Comparator<CSQueue>() {
	//比较队列资源使用量比例，因为队列getUsedCapacity得到的就是资源使用比例
    @Override
  ...
  private CapacitySchedulerConfiguration conf;//这里有层级资源组织方式
  private Configuration yarnConf;

  private Map<String, CSQueue> queues = new ConcurrentHashMap<String, CSQueue>();//队列集合

  private Map<NodeId, FiCaSchedulerNode> nodes = 
      new ConcurrentHashMap<NodeId, FiCaSchedulerNode>();//node集合，以Scheduler角度看的node

  private Resource clusterResource = 
    RecordFactoryProvider.getRecordFactory(null).newRecordInstance(Resource.class);//集群资源
  ...
  private ResourceCalculator calculator;//资源计算器
  private boolean usePortForNodeName;

  private boolean scheduleAsynchronously;//是否启用异步线程，默认是true
  ...
  public synchronized void
    ...
    if (!initialized) {
      ...
      initializeQueues(this.conf);//初始化队列 ，构建树
      ...
      if (scheduleAsynchronously) {
        ...
        asyncSchedulerThread.start();//启动异步执行线程
      }
      ...
      try {
        ...
        reinitializeQueues(this.conf);//如果已经init了，这里重新加载配置文件，用来实现重新配置队列
      } catch (Throwable t) {
        this.conf = oldConf;
  ...
  private void initializeQueues(CapacitySchedulerConfiguration conf)
    ...
    root = 
        parseQueue(this, conf, null, CapacitySchedulerConfiguration.ROOT, 
            queues, queues, noop);//递归构建树，返回根节点
  ...
  static CSQueue parseQueue(
    ...
    String[] childQueueNames = 
      conf.getQueues((parent == null) ? 
          queueName : (parent.getQueuePath()+"."+queueName));//从根节点到当前节点的路径名 作为关键字，获取子节点的名字数组
    if (childQueueNames == null || childQueueNames.length == 0) {//如果没有子节点
      if (null == parent) {
        throw new IllegalStateException(
            "Queue configuration missing child queue names for " + queueName);
      }
      queue = 
          new LeafQueue(csContext, queueName, parent,oldQueues.get(queueName));//当前节点是叶子节点，新建一个叶子节点LeafQueue
      
      // Used only for unit tests
      queue = hook.hook(queue);
    } else {
      ParentQueue parentQueue = 
        new ParentQueue(csContext, queueName, parent,oldQueues.get(queueName));//新建一个非叶子节点ParentQueue
    ...
    queues.put(queueName, queue);//queues是树的所有<路径，节点>集合
  ...
  private synchronized void addApplication(ApplicationId applicationId,
    ...
    // santiy checks.
    CSQueue queue = getQueue(queueName);//找到要挂载的队列
    if (queue == null) {
    ...
    if (!(queue instanceof LeafQueue)) {//正常逻辑应该是LeafQueue
      String message = "Application " + applicationId + 
    ...
    // Submit to the queue
    try {
    	//把这个application挂载到queue这个叶子节点上。
    	//capacity Scheduler把资源构建为树状，app挂在叶子节点上
    ...
    applications.put(applicationId, application);//添加到applications，持有application这些对象
  ...
  private synchronized void addApplicationAttempt(
    ...
    application.setCurrentAppAttempt(attempt);//设置当前的运行实例

    queue.submitApplicationAttempt(attempt, application.getUser());//挂载appAttempt
  ...
  public Allocation allocate(ApplicationAttemptId applicationAttemptId,
    ...
    FiCaSchedulerApp application = getApplicationAttempt(applicationAttemptId);//找出caller
    if (application == null) {
    ...
    synchronized (application) {
      ...
      if (!ask.isEmpty()) {
        ...
        // Update application requests 更新资源请求
      ...
      //如果有要新分配的container，设置这些新的container的一些参数，并发出对应container的RMContainerEventType.ACQUIRED事件。
      //返回一个allocation
      //如果有要新分配的container，返回的allocation里面有要分配的container列表。并发出一个对应container的RMContainerEventType.ACQUIRED
      //否则，container列表是空，而且不会有对应的RMContainerEventType.ACQUIRED事件
  ...
  //更新Node在RM中的信息，在这个node上分配新的container，释放完成的container，并发送相应的事件
  private synchronized void nodeUpdate(RMNode nm) {
    ...
    for (ContainerStatus launchedContainer : newlyLaunchedContainers) {
      containerLaunchedOnNode(launchedContainer.getContainerId(), node);//发出RMContainerEventType.LAUNCHED事件
    }
  ...
  private synchronized void allocateContainersToNode(FiCaSchedulerNode node) {
    ...
    // 1. Check for reserved applications 查找预订保留的Applications，就是找已经申请还没分配的app
    // 2. Schedule if there are no reservations

    RMContainer reservedContainer = node.getReservedContainer();
    if (reservedContainer != null) {//如果有之前未能分配container而预留的
      ...
      CSAssignment assignment = queue.assignContainers(clusterResource, node);//分配container
      
      RMContainer excessReservation = assignment.getExcessReservation();//超额的
  ...
  public void handle(SchedulerEvent event) {
    ...
    case NODE_UPDATE:
    {
      ...
      if (!scheduleAsynchronously) {
        allocateContainersToNode(getNode(node.getNodeID()));//分配container，给提出请求的appAttempt分配container
      }
    }
    break;
    case APP_ADDED:
  ...
  private synchronized void addNode(RMNode nodeManager) {
    ...
    if (scheduleAsynchronously && numNodeManagers == 1) {//停止休眠
