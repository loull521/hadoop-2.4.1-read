  ...
  private static final class ScheduleTransition
    ...
    public RMAppAttemptState transition(RMAppAttemptImpl appAttempt,
      ...
      if (!appAttempt.submissionContext.getUnmanagedAM()) {
    	  //在默认环境下返回false。也就是AM的container可能出现在任意的NM节点上。
    	  //如果getUnmanagedAM返回true，就是让AM运行在客户端一个单独线程中，不由RM启动和销毁
    	  //所以一般情况是进入这里代码
        ...
        //注意这里的ResourceRequest.ANY，表示“*”，其它有local、本机架。ANY表示可以分配这个container到任意NM节点
        ...
        Allocation amContainerAllocation = appAttempt.scheduler.allocate(
        	...
        	//同步调用Scheduler的allocate方法，这里阻塞了吗？
        	//分配资源是异步分配的，所以这里不会IO阻塞，而是返回一个没有container的Allocation。
