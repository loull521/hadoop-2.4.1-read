 ...
 * scheduler.<br>
 * 以Scheduler的角度维护应用程序的运行实例信息。<br>
 * 持有attemptId、queue、user、rmcontext,
 * 是SchedulerApplicationAttempt的子类
 */
@Private
@Unstable
public class FiCaSchedulerApp extends SchedulerApplicationAttempt {
  ...
  private final Set<ContainerId> containersToPreempt =
    new HashSet<ContainerId>();//要抢占的container
  ...
  //创建并分配RMContainerImpl实例，发出RMContainerEventType.START事件
   ...
   * application.
   * 返回一个allocation。<br>
   * 如果有要新分配的container，返回的allocation里面有要分配的container列表。并发出一个对应container的RMContainerEventType.ACQUIRED。<br>
  ...
  public synchronized Allocation getAllocation(ResourceCalculator rc,
    ...
    //否则allocation中要分配的container列表为空
