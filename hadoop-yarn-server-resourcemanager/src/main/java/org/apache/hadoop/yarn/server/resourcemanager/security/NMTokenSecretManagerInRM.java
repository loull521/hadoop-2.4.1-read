...
public class NMTokenSecretManagerInRM extends BaseNMTokenSecretManager {
  ...
  public NMToken createAndGetNMToken(String applicationSubmitter,
	  ...
	  //方法返回NMToken，每个NM唯一。
	  //同一个node里的不同container调用这个方法，第一次会返回nmtoken，后面就返回null
    try {
      this.readLock.lock();
      HashSet<NodeId> nodeSet = this.appAttemptToNodeKeyMap.get(appAttemptId);//获取已给appAttempt分配的node
      NMToken nmToken = null;
      if (nodeSet != null) {
        if (!nodeSet.contains(container.getNodeId())) {//如果container对应的node的id不在nodeSet中，则新生成一个。否则，返回的nmToken为null
