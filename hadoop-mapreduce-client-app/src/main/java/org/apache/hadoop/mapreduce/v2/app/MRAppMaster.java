...
public class MRAppMaster extends CompositeService {
  ...
  protected void serviceInit(final Configuration conf) throws Exception {
    ...
    jobId = MRBuilderUtils.newJobId(appAttemptID.getApplicationId(),
        appAttemptID.getApplicationId().getId());//利用appId，appAttemptId生成jobId
  ...
  protected void serviceStart() throws Exception {
    ...
    job = createJob(getConfig(), forcedState, shutDownMessage);//创建JobImpl实例
    ...
    if (!errorHappenedShutDown) {
      ...
      JobEvent initJobEvent = new JobEvent(job.getID(), JobEventType.JOB_INIT);//发送INIT事件
      // Send init to the job (this does NOT trigger job execution)
      // This is a synchronous call, not an event through dispatcher. We want
      // job-init to be done completely here.直接调用，同步方法
    ...
    startJobs();//启动Job，发送JobEventType.JOB_START事件
  }
