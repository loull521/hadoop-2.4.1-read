  ...
  private class Container {
    ...
    public synchronized void launch(ContainerRemoteLaunchEvent event) {
      ...
      try {
        ...
        proxy = getCMProxy(containerMgrAddress, containerID);//获取客户端代理

        // Construct the actual Container
        ContainerLaunchContext containerLaunchContext =
          event.getContainerLaunchContext();//从事件中获取启动container需要的信息

        // Now launch the actual container
        StartContainerRequest startRequest =
            StartContainerRequest.newInstance(containerLaunchContext,
              event.getContainerToken());//实例化一个启动请求
        List<StartContainerRequest> list = new ArrayList<StartContainerRequest>();
        list.add(startRequest);
        StartContainersRequest requestList = StartContainersRequest.newInstance(list);
        StartContainersResponse response =
            proxy.getContainerManagementProtocol().startContainers(requestList);//RPC调用，要求启动containers
        ...
        context.getEventHandler().handle(
            new TaskAttemptContainerLaunchedEvent(taskAttemptID, port));//发送TaskAttemptEventType.TA_CONTAINER_LAUNCHED事件
