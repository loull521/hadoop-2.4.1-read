...
public class RPC {
  ...
  /**
   * Get all superInterfaces that extend VersionedProtocol<br>
   * 获取所有的父接口
  ...
  // 保存protocol 与 RpcEngine 的映射关系，不同protocol可以用不同的RpcEngine
  ...
  /**
   * Set a protocol to use a non-default RpcEngine.<br>
   * 设置RpcEngine，比如AvroRpcEngine、ProtobufRpcEngine
  ...
  // 从配置文件中获取 配置的 RpcEngine，如果没有配置，默认为WritableRpcEngine
...
//									ProtobufRpcEngine.class);
      ...
      LOG.info("RPC# " + "protocol name: " + protocol.getName());
      LOG.info("RPC# " + "engine: " + engine.getClass().getName());
   ...
   * and a set of methods that are supported by the server<br>
   * 返回一个客户端ProtocolProxy，有server端的支持的方法
   * 
   * @param protocol protocol class
   * @param clientVersion client version
   * @param addr remote address
  ...
  /**
   * Get a proxy connection to a remote server
   * T是protocol协议接口，返回的是protocol的proxy
   * 
   * @param protocol protocol class
   * @param clientVersion client version
   * @param addr remote address
   ...
   * Get a protocol proxy that contains a proxy connection to a remote server
   * and a set of methods that are supported by the server<br>
   * 用RpcEngine的getProxy方法得到ProtocolProxy
   * 
   * @param protocol protocol
   * @param clientVersion client's version
   * @param addr server address
  ...
  /**
   * Class to construct instances of RPC server with specific options.<br>
   * 构造一个Builder对象
   */
  public static class Builder {
    ...
    /**
     * Build the RPC Server. <br>
     * 构建server之前必须提供conf配置，很多参数的在这里设置<br>
     * protocol：要使用的协议（接口）,RPC本来就是框架，给出要通信的协议，也就是client要调用server的方法。<br>
     * instance：protocol协议接口的实现类，server端必须有协议的具体实现类，否则client要你有何用？
    ...
    public Server build() throws IOException, HadoopIllegalArgumentException {
      ...
      //用protocol的类型、conf获得一个RPCEngine：RPCEngine是一个序列化框架，这里一般得到WritableRpcEngine或ProtobufRpcEngine
      //进入WritableRpcEngine看看有getServer
  ...
  public abstract static class Server extends org.apache.hadoop.ipc.Server {
   ...
   //server的成员域，map的列表，key是protocol_name+protocol_version, value:=protocol_class+protocol_impl
   //每个map对应一个rpcKind
   ...
   VerProtocolImpl getHighestSupportedProtocol(RpcKind rpcKind, 
    ...
    private void initProtocolMetaInfo(Configuration conf) {
      RPC.setProtocolEngine(conf, ProtocolMetaInfoPB.class,
          ProtobufRpcEngine.class);//设置一个非默认的RpcEngine
