 ...
 * a port and is defined by a parameter class and a value class.<br>
 * Server请求处理过程：建立连接，接收请求，获取call对象，处理call对象，返回请求。<br>
 * Server的start()方法启动了responder、listener、handlers线程<br>
 * 启动Listener线程，NIO方式监听accept请求。每接受一个accept请求，启动一个reader线程NIO方式读取数据。<br>
 * reader把请求转化为一个call，放到callQueue里面，<br>
 * handlers会不停得去callQueue去取call,处理完发送回客户端。如果发送失败，再交给responder用NIO方式返回。<br>
 * callQueue是一个阻塞队列，这里相当于实现一个生产者消费者模式
 * 
 * @see Client
 */
public abstract class Server {
  ...
  /**
   * RPC序列化方式和序列化类对应表，比如protocolbuf。。。
   */
   ...
   *  the rpc request.<br>
   *  这里一般用这个类名：Invocation.class，函数调用信息打包成可序列化的对象<br>
   *  反序列化rpc请求
   *  @param rpcInvoker - use to process the calls on SS.调用RPC请求的方法
   */
   ...
   *  一次请求被封装成一个call，server端存储客户端发送过来的call*/
   ...
   *  Listener启动一个线程，select方式监听客户端连接*/
  private class Listener extends Thread {
    
    private ServerSocketChannel acceptChannel = null; //the accept channel
    private Selector selector = null; //the selector that we use for the server
    private Reader[] readers = null;//read池
    ...
    public Listener() throws IOException {
      ...
      // create a selector;
      selector= Selector.open();//这个selector专门监听accept请求，即连接请求
    ...
    private class Reader extends Thread {
      ...
      private final Selector readSelector;//这个selector监听读请求
    ...
    public void run() {
      ...
      connectionManager.startIdleScan();//启动scan idle Connection，把空闲一定时间的Connection关闭
      while (running) {
        SelectionKey key = null;
        try {
        ...
        } catch (OutOfMemoryError e) {//直接捕获OOM错误
    ...
    void doAccept(SelectionKey key) throws InterruptedException, IOException,  OutOfMemoryError {
      ...
      while ((channel = server.accept()) != null) {
        ...
        Reader reader = getReader();//从reader池中取出一个reader
        Connection c = connectionManager.register(channel);//把Connection注册到ConnManager
    ...
    void doRead(SelectionKey key) throws InterruptedException {
      ...
      try {
    	  //readAndProcess方法调用Server.Connection类中的processOneRpc方法。
    	  //processOneRpc中读取参数封装成call，然后放到callQueue中
   ...
   *  响应RPC请求，请求处理完毕，由Responder发送给客户端<br>
   *  这里也是用NIO
   */
  private class Responder extends Thread {
    ...
    private void doRunLoop() {
      ...
      while (running) {
        try {
          waitPending();     // If a channel is being registered, wait. 如果有Connection等着注册，线程wait
  ...
  public class Connection {
    ...
    private volatile int rpcCount = 0; // number of outstanding rpcs 未完成的rpcs
    ...
    /**
     * readAndProcess方法调用Server.Connection类中的processOneRpc方法。<br>
     * processOneRpc中读取参数封装成call，然后放到callQueue中<br>
     * 最多只处理一个RPC
     * @return
...
//          LOG.info("loull - readAndProcess() head-4-bytes:" + new String(dataLengthBuffer.array()));
        ...
        if (data.remaining() == 0) {//表示正确读取dataLength长度的数据
    ...
    /**
     * 兼容版本2，ClientVersion == 2
     */
    private void compatibleVersion2() {
     ...
     * 从Connection head 中读取Connection context
    ...
    private void processOneRpc(byte[] buf)
    	...
    	LOG.info("loull - processOneRpc!!!!!!!!!!!!!!!!!");
      int callId = -1;
      int retry = RpcConstants.INVALID_RETRY_COUNT;
      try {
        final DataInputStream dis =
            new DataInputStream(new ByteArrayInputStream(buf));//把请求的字节码数组包装到stream
        final RpcRequestHeaderProto header =//TODO 从dis中解析出header
            decodeProtobufFromStream(RpcRequestHeaderProto.newBuilder(), dis);//从stream中读出header部分，还有context部分
        callId = header.getCallId();//如果现在没有active的call，则返回一个负值
        ...
        if (callId < 0) { // callIds typically used during connection setup
        	LOG.info("loull - processOneRpc callId<0？" + callId);
          ...
          LOG.info("loull - processOneRpc callId>=0？" + callId);
          processRpcRequest(header, dis);//处理dis中还剩的context部分，解析的时候需要header的信息
        }
    ...
    private void processRpcRequest(RpcRequestHeaderProto header,
      ...
      Class<? extends Writable> rpcRequestClass = //根据ProtocolKind（Writable/protobuf），取得对应的Invocation.class
          getRpcRequestWrapper(header.getRpcKind());//rpcRequestClass(也就是Invocation.class)是函数调用信息的序列化对象的class
      										//有了这个class之后就可以反射得到rpc请求对象
      if (rpcRequestClass == null) {
      ...
      Writable rpcRequest;//Writable：可序列化对象
      try { //Read the rpc request
        rpcRequest = ReflectionUtils.newInstance(rpcRequestClass, conf);//用反射工具得到RPC请求对象
        					//默认情况下是WritableRpcEngine.Invocation对象，这时候这个对象刚创建，成员域为空，没有具体值
        rpcRequest.readFields(dis);//填充成员域，这时rpcRequest已经是一个完整得RPC请求
      ...
      Call call = new Call(header.getCallId(), header.getRetryCount(),
          rpcRequest, this, ProtoUtil.convert(header.getRpcKind()), header
              .getClientId().toByteArray());//用完整的RPC请求对象封装成Call
   ...
   * <br>启动一个线程，处理请求，循环阻塞读取callQueue中的call对象 
   * <br>当没有call对象没有到达，或者说队列中没有call对象，线程会阻塞*/
        ...
        try {
          ...
          try {
            ...
            if (call.connection.user == null) {
              value = call(call.rpcKind, call.connection.protocolName, call.rpcRequest, 
                           call.timestamp);//call()方法需要在server的子类中实现
            } else {
              value = 
          ...
          synchronized (call.connection.responseQueue) {
            ...
            setupResponse(buf, call, returnStatus, detailedErr, 
                value, errorClass, error); //把RPC调用结果序列化到buf中
  ...
  /** Called for each call.<br>
   *  最后会在RpcEngine的子类（比如WritableRpcEngine）里面的WritableRpcInvoker(RPC.RpcInvoker的实现类)的call方法里面实现<br>
   *  RPC.Server类里面实现了call：getRpcInvoker()里面实现。WritableRpcEngine.Server类里面有个RpcInvoker的实现类：WritableRpcInvoker*/
  ...
  private class ConnectionManager {
    ...
    final private Timer idleScanTimer;//定时器，定时scan idle Connection
    final private int idleScanThreshold;//close的数量阈值，到了这个阈值就不再close了，除非scanAll==true
    final private int idleScanInterval;//scan idle connection间隔
    final private int maxIdleTime;//最大idle时间，超出这个时间会被close
    final private int maxIdleToClose;//一轮closeIdle中最大可以close的idle Connection
    ...
    //通过channel获取Connection，并且把这Connection加到set中
    ...
    boolean close(Connection connection) {
      boolean exists = remove(connection);//线程集合set中移除
      if (exists) {
        if (LOG.isDebugEnabled()) {
        ...
        // to possible races
        connection.close();//关闭Connection，用了exists，为了防止double-closeing，可以少用一个同步处理
      }
      return exists;
    }
    ...
    synchronized void closeIdle(boolean scanAll) {
      long minLastContact = Time.now() - maxIdleTime;//小于这个时间的会被close
      ...
      for (Connection connection : connections) {
        ...
        if (connection.isIdle() &&//connection上rpc的数量==0？为0则返回true
            connection.getLastContact() < minLastContact &&//
            close(connection) &&//关闭连接
            !scanAll && (++closed == maxIdleToClose)) {//如果closed的数量达到预设的最大值，就跳出循环，不继续close了
          break;
        }
      }
    }
    ...
    private void scheduleIdleScanTask() {
      ...
      TimerTask idleScanTask = new TimerTask(){
        ...
        public void run() {
          ...
          try {
            closeIdle(false);//不scan所有Connection
          } finally {
            // explicitly reschedule so next execution occurs relative
            // to the end of this scan, not the beginning
            scheduleIdleScanTask();//TODO ????看不懂啊，原线程应该调用这个方法之后很快就结束了吧？
          }
        }
      };
      idleScanTimer.schedule(idleScanTask, idleScanInterval);//默认10s后开始扫描
    }
  }
}
